...
def generate_report_figures():
    ...
    rgb = ls.shade(dem.elevation, cmap=plt.cm.terrain, blend_mode='overlay', vert_exag=3)

    extent = [dem.x_coords[0]/1000, dem.x_coords[-1]/1000, dem.y_coords[0]/1000, dem.y_coords[-1]/1000]
    im = ax.imshow(rgb, extent=extent, origin='lower')

    # Overlay Catchment Mask Contour
    mask = catchment['catchment_mask'].astype(float)
    cs = ax.contour(mask, levels=[0.5], extent=extent, origin='lower', colors=['#00E5FF'], linewidths=2.5)
    
    # Overlay Stream Network
    stream_mask = results['flow_accumulation'] >= np.percentile(results['flow_accumulation'], 98.0)
    # Scatter stream cells
    sy, sx = np.where(stream_mask)
    ax.scatter(dem.x_coords[sx]/1000, dem.y_coords[sy]/1000, s=1, c='#0288D1', alpha=0.6, label='Drainage Network')

    # Plot Recommended Pond Location
    top_e = top_site['utm_coordinates']['easting'] / 1000
    top_n = top_site['utm_coordinates']['northing'] / 1000
    ax.scatter(top_e, top_n, s=160, c='#00E676', edgecolors='#000000', linewidth=2, zorder=10, label=f"Recommended Pond Site (Score: {top_site['suitability_score']})")
    ...
    for site in candidate_sites[1:]:
        ...
        ax.scatter(se, sn, s=90, c='#FF9100', edgecolors='#000000', linewidth=1.5, zorder=9)

    ax.set_title("Terrain Elevation, Drainage Streams & Delineated Watershed Catchment", fontsize=13, fontweight='bold', pad=12)
    ...
    im1 = axs[0, 1].imshow(np.log1p(results['flow_accumulation']), cmap='Blues', origin='lower')
    axs[0, 1].set_title("B. Log-Scaled Flow Accumulation (Drainage Network)", fontweight='bold')
    ...
    im3 = axs[1, 1].imshow(results['depression_depth'], cmap='PuBuGn', origin='lower', vmax=6)
    axs[1, 1].set_title("D. Topographic Depressions / Natural Storage Bowls (m)", fontweight='bold')
