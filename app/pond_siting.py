...
class PondSitingEngine:
    ...
    def find_optimal_sites(self,
                           ...
                           boundary_margin_cells: int = 8) -> List[Dict[str, Any]]:
        """
        ...
        twi = np.log((flow_acc * dem.resolution_m) / (np.tan(np.maximum(slope_rad, 0.01))))
        ...
        for r, c in candidate_grids:
            if not valid_mask[r, c]:
                continue
            if any(abs(r - ur) < min_cell_spacing and abs(c - uc) < min_cell_spacing for ur, uc in unique_candidates):
                continue
        ...
        for r, c in unique_candidates:
            ...
            if catchment_area_ha >= 10.0:
                rationale_parts.append(f"Substantial upstream drainage ({catchment_area_ha:.1f} ha)")
            else:
                rationale_parts.append(f"Moderate upstream catchment ({catchment_area_ha:.1f} ha)")

            if dep_d >= 1.0:
                rationale_parts.append(f"Located in a natural topographic bowl ({dep_d:.1f}m depth) reducing excavation")
            elif dep_d > 0.1:
            ...
            if slp < 3.0:
                rationale_parts.append(f"very gentle bed slope ({slp:.1f}%) for stable embankment")
            elif slp < 8.0:
    ...
    def compute_design_recommendations(self,
                                       catchment_area_sq_m: float,
                                       annual_runoff_m3: float,
                                       depression_depth_m: float = 1.0,
                                       target_pond_depth_m: float = 3.0) -> Dict[str, Any]:
        """
        ...
        excavation_vol_m3 = required_surface_area_sq_m * effective_excavation_depth_m * 0.75
        excavation_savings_pct = round(((depth_m - effective_excavation_depth_m) / depth_m) * 100, 1)
        ...
        return {
            ...
            'recommended_surface_area_hectares': round(required_surface_area_sq_m / 10000.0, 3),
            ...
            'utilization_potential': {
                ...
                'estimated_annual_refill_cycles': round(min(5.0, annual_runoff_m3 / target_capacity_m3), 1)
            },
            'construction_notes': [
