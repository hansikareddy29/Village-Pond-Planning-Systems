...
def test_synthetic_straight_downhill(hydrology):
    elev = np.array(
        [[10.0, 9.0, 8.0], [9.0, 8.0, 7.0], [8.0, 7.0, 6.0]], dtype=np.float64
    )
...
def test_synthetic_confluence_merge(hydrology):
    elev = np.array(
        [[10.0, 5.0, 10.0], [8.0, 4.0, 8.0], [7.0, 3.0, 7.0], [6.0, 2.0, 6.0]],
        dtype=np.float64,
    )
...
def test_synthetic_catchment_delineation(hydrology):
    ...
    catchment_info = hydrology.delineate_catchment(
        pour_point_grid=(2, 1), flow_dir=flow_dir, dem=dem
    )
...
def test_area_unit_consistency(hydrology):
    area_sq_m = 100000.0
    runoff = hydrology.estimate_runoff(
        area_sq_m, annual_rainfall_mm=1000.0, runoff_coefficient=0.35
    )
